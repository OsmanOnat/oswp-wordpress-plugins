<?php
/**
 * Require js / css enqueue admin
 */
function add_enqueue_custom_style_script()
{ 
    wp_register_style(
        "oswp_whatsapp_custom_style_1",
        WHATSAPP_PLUGINS_URL_INC . "css/custom-admin.css",
        false,
        null,
    );

    wp_register_script( 
        'oswp_whatsapp_custom_script_1',
        WHATSAPP_PLUGINS_URL_INC . "js/admin.js",
        array(), 
        null, 
        true
    );

    wp_enqueue_style( "oswp_whatsapp_custom_style_1" );
    wp_enqueue_script( "oswp_whatsapp_custom_script_1" );
}
add_action( 'admin_enqueue_scripts', 'add_enqueue_custom_style_script' );
        ...
        else
        {
            if( $temp_file_check[ "type_check" ] == true )
            {
                move_uploaded_file(
                    ...
                    WHATSAPP_PLUGINS_ADMIN_IMAGE_UPLOAD . $_FILES["icon"]['name']
                );
...
<?php
    ...
    <tbody>
        ...
        <?php require WHATSAPP_PLUGIN_PATH . "inc/admin/admin_plugin_usage_description.php" ?>
